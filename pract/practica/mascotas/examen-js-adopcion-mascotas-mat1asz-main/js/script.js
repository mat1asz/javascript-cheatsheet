...
async function obtenerAnimales() {
  try {
    ...
    if (!respuesta.ok) throw new Error('No se pudo obtener el catálogo de animales');
...
async function obtenerEspecies() {
  try {
    ...
    if (!respuesta.ok) throw new Error('No se pudo obtener las especies');
    const datos = await respuesta.json();
    return datos;
  } catch (error) {
    console.error(error);
...
//GET http://localhost:3000/api/animales/:id : Devuelve la ficha de un animal por su ID numéric
...
function renderizarSelectArray(lista, elementoSelect) {
  if (!lista) return;
  //elementoSelect.innerHTML = '';  lo saco porque en el select hay un option (todas las especies)
  lista.forEach((item) => {
...
<div class="animal-card">
  ...
  <div class="card-body">
    ...
    <p class="card-breed">
      ${item.raza} &bull; ${item.sexo}
    </p>
    <p class="card-shelter">
      <i class="fas fa-home"></i> ${item.refugio}
    </p>
...
function renderizarAnimales(lista) {
  ...
  lista.forEach((item) => {
    catalogoGrid.innerHTML += crearTarjetaAnimal(item); //function que acabo de hacer arriba
  });
}
//fin del commit 3
//------------------------------------------------------------------------------------------
...
function aplicarFiltros() {
  ...
  //no habia segundo filtro
  //no habia input de busqueda
