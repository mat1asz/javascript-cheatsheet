...
function renderizarSelectArray(lista, elementoSelect) {
  if (!lista) return;
  elementoSelect.innerHTML = ''; //verificar aca si en el select hay un option adentro de la etiqueta
  lista.forEach((item) => {
...
__GRID__.addEventListener('click', async (evento) => {
  if (evento.target.classList.contains('__clase_boton_accion__')) {
    // ↓ CAMBIA: btn-ordenar, btn-comprar, btn-adoptar, etc.
